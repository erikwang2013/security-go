...
- [x] **Comprehensive code review** — 42 Go source files, 8 packages
- [x] **Bug fix #1** — `storage/file.go`: JSON serialization errors were silently ignored → now checks the error and returns it
- [x] **Bug fix #2** — `httpval/content_type.go`: empty AllowList allowed all Content-Types → deny-all default
- [x] **Bug fix #3** — `protocol/xxe.go`: `&[a-z]+;` falsely matched legitimate HTML entities → narrowed to a list of known malicious protocols
- [x] **Added httpval tests** — 32 test cases covering 5 detectors (BodySize, ContentType, CSRFOrigin, IPBlacklist, Method)
- [x] **Full test run** — `go test -count=1 ./...` 7/7 packages pass, `go vet` zero warnings
...
|------|------|------|
| RegisterAll in `security.go` | Separate package `all/all.go` | Avoids circular imports; httpval depends on storage but other detectors do not |
| Redis in root go.mod | Submodule `storage/redis/` | Isolates the optional dependency |
| Unified pointer receivers | protocol package used value receivers | ✅ All changed to pointer receivers in the v2 review |
| Tasks 4-12 Build & Commit | No per-step commits | All code was implemented at once |
...
> Full report: `../reports/2026-07-29-code-review-report-v2.md`
